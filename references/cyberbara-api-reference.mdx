...
CyberBara exposes one public model ID, `grok-imagine-image-2`. Use
`options.mode: "image-edit"` for image editing. Layer separation is not exposed
as a CyberBara workflow for this model.

| Field                    | Type                 | Required | Allowed / Default                                  | Notes |
| ------------------------ | -------------------- | -------- | -------------------------------------------------- | ----- |
| `prompt`                 | `string`             | Yes      | non-empty | Required for `text-to-image` and `image-edit` |
| `options.mode`           | `string`             | No       | `text-to-image` / `image-edit` (default `text-to-image`) | Selects the internal KIE route |
...
Pricing: text-to-image costs 4 credits per image, and image edit costs 4
credits per image.
